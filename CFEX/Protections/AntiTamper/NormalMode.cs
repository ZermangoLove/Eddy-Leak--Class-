...
using Protector.Protections.ControlFlow;
...
namespace Protector.Protections.AntiTamper
{
 internal class NormalMode : IModeHandler
 {
  ...
  public ModuleDef HandleInject(ModuleDef module, ProtectorContext ctx)
  {
   random = new RandomGenerator(ctx.random_generator.RandomBytes(32));
   ...
   ctx.AntiTampKeys.AddKeys(KEYS.ToArray());

   

   deriver = new DynamicDeriver();

   deriver.Init(ctx, random);

   TypeDef initType = DnLibHelper.GetRuntimeType("Protector.Runtime.AntiTamperNormal");

   IEnumerable<IDnlibDef> members = null;
   if (ctx.StubType != null)
   {
    members = InjectHelper.Inject(initType,ctx.StubType, module);
   }
   members = InjectHelper.Inject(initType, module.GlobalType, module);

   var initMethod = (MethodDef)members.Single(m => m.Name == "Initialize"); //This is Runtime method for decode antitamper

   var getPayloadMethod = (MethodDef)members.Single(m => m.Name == "GetThePayload");
   getPayloadMethod.Name = ctx.random_generator.GenerateString();

   var recorverKeyMethod = (MethodDef)members.Single(m => m.Name == "RestoreKey");
   recorverKeyMethod.Name = ctx.random_generator.GenerateString();

   var decodePayloadMethod = (MethodDef)members.Single(m => m.Name == "DecodePayload");
   decodePayloadMethod.Name = ctx.random_generator.GenerateString();

   initMethod.Body.SimplifyMacros(initMethod.Parameters);
   List<Instruction> instrs = initMethod.Body.Instructions.ToList();
   instrs[1].Operand = module.GlobalType;
   for (int i = 0; i < instrs.Count; i++)
   {
     ...
     if (instr.OpCode == OpCodes.Call)
    {
         ...
         method.Name == "Crypt")
     {
      ...
      instrs.RemoveAt(i - 1);
      instrs.RemoveAt(i - 2);
      instrs.InsertRange(i - 2, deriver.EmitDerivation(initMethod, ctx, (Local)ldDst.Operand, (Local)ldSrc.Operand));
     }
    }
   }
   ...
   MutationHelper.InjectKeys(initMethod, new[] { 0 }, new[] { AntiTamperProtection.AntitampersUsed });

   initMethod.Name = ctx.random_generator.GenerateString();
   ...
   if(AntiTamperProtection.VirtualizeAntitamper)
   {
    VM vm = new VM();
    vm.AddTarget(initMethod);
    vm.AddTarget(recorverKeyMethod);
    vm.AddTarget(getPayloadMethod);
    vm.Initialize(module, ctx);
    AntiTamperProtection.VirtualizeAntitamper = false;
   }
   if(AntiTamperProtection.VirtualizeCctor)
   {
    VM vm = new VM();
    vm.AddTarget(recorverKeyMethod);
    vm.AddTarget(getPayloadMethod);
    //vm.AddTarget(decodePayloadMethod);
    vm.AddTarget(module.GlobalType.FindDefaultConstructor());
    vm.Initialize(module, ctx);
   }

   if(AntiTamperProtection.VirtualizeAntitamperAndCctor)
   {
    VM vm = new VM();
    vm.AddTarget(initMethod);
    vm.AddTarget(module.GlobalType.FindDefaultConstructor());
    vm.Initialize(module, ctx);
    AntiTamperProtection.VirtualizeAntitamperAndCctor = false;
   }

   //ctx.AntiTamperMethods.Add(decodePayloadMethod);
   //ctx.AntiTamperMethods.Add(initMethod);

   AntiTamperProtection.AntitampersUsed++;

   return module;
  }

  public void HandleMD(List<MethodDef> input_methods, ProtectorContext context)
  {
  ...
  /* Event for proccess methods and write sections to assembly */
