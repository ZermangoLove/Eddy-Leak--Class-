...
namespace Eddy_Protector_Protections.Protections.AntiTamper
{
	internal class NormalMode : IModeHandler
	{
		...
		public void HandleInject(Context ctx)
		{
			...
			TypeDef initType = Utils.GetRuntimeType("Eddy_Protector_Runtime.AntiTamperNormal");
			IEnumerable<IDnlibDef> members = InjectHelper.Inject(initType, ctx.CurrentModule.GlobalType, ctx.CurrentModule);
			...
			for (int i = 0; i < instrs.Count; i++)
			{
				...
				{
					instr.Operand = ctx.CurrentModule.GlobalType;
				}
			...
			initMethod.Name = ctx.generator.GenerateNewNameChinese();
   //ctx.runtime_math.DoMathProtection(initMethod, ctx);
   //ctx.runtime_antidnspy.DoAntiDnspy(initMethod, ctx);

   //ctx.runtime_protect.runtime_math.DoMathProtection(initMethod, ctx); //Works!!
   //ctx.runtime_protect.runtimeControlFlow2.DoControlFlow(initMethod, ctx);

   //ctx.analyzer.targetCtx.methods_virtualize.Add(initMethod);
		...
		public void HandleMD(List<MethodDef> input_methods,Context context)
		{
  ...
  /* Event for proccess methods and write sections to assembly */
		...
		void CreateSections(ModuleWriterBase writer)
		{
			...
			//char[] name = "Eddy^CZ2018".ToArray();

			//for (int f = 0; f < 8; f++)
			//{
			//	nameBuffer[f] = Convert.ToByte(name[f]);
			//}
