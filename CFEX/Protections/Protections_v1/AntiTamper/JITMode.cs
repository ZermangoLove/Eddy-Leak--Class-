...
namespace Eddy_Protector_Protections.Protections.AntiTamper
{
	internal class JITMode : IModeHandler
	{
		...
		public void HandleInject(Context context)
		{
			...
			TypeDef initType = Utils.GetRuntimeType("Eddy_Protector_Runtime.AntiTamperJIT");
			...
			cctor = context.CurrentModule.GlobalType.FindOrCreateStaticConstructor();

			cctorRepl = new MethodDefUser(context.generator.GenerateNewName(), MethodSig.CreateStatic(context.CurrentModule.CorLibTypes.Void));
		...
		public void HandleMD(List<MethodDef> methods_input,Context context)
		{
		...
		void CreateSection(ModuleWriterBase writer)
		{
			...
			// save methods
			foreach (MethodDef method in context.analyzer.targetCtx.methods_targets)
			{
				if (!method.HasBody)
					continue;
