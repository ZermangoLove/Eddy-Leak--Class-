...
{
 /// <summary>
 /// This is a Special protection to Preprotect input code.
 /// </summary>
 public class Prepacker
 {
  public byte[] Prepack(byte[] InputProgram)
  {
   var secured = Protect(InputProgram);
   return secured;
  }

  private byte[] Protect(byte[] Target)
  {
   ...
   try
   {
    File.WriteAllBytes(tar_p, Target);
   } 
   catch
   {
   ...
   if(th_ok)
   {
    result = File.ReadAllBytes(Path.Combine(paths[0],"OUT/"+ tar_p_abs));
    if(result.Length != 0)
    {
