...
{
 public class Constants2Protection : ProtectionPhase
 {
  ...
  public void FirstPhase(Context ctx)
  {
   ...
   FieldDefinition constTbl = new FieldDefinition(ctx.generator.GenerateNewNameChinese(), Mono.Cecil.FieldAttributes.Static | Mono.Cecil.FieldAttributes.CompilerControlled, mod.Import(typeof(Dictionary<uint, object>)));
   modType.Fields.Add(constTbl);
   FieldDefinition constBuffer = new FieldDefinition(ctx.generator.GenerateNewNameChinese(), Mono.Cecil.FieldAttributes.Static | Mono.Cecil.FieldAttributes.CompilerControlled, mod.Import(typeof(byte[])));
                         ...
                         FieldDefinition constTbl, FieldDefinition constBuffer)
  {
   ...
   if (lzma == null)
   {
    ...
    lzma.Name = ctx_.generator.GenerateNewNameChinese();
    ctx_.analyzer.targetCtx.types_mono_names.Add(lzma.Name);
    mod.Types.Add(lzma);
   }
   ...
   //init.DeclaringType = mod.GetType("<Module>");
   {
    ...
    m.Name = ctx_.generator.GenerateNewNameChinese();
   ...
   for (int i = 0; i < typeDefCount; i++)
   {
    TypeDefinition typeDef = new TypeDefinition(
        "", ctx_.generator.GenerateNewNameChinese(),
    ...
    ctx_.analyzer.targetCtx.types_mono_names.Add(typeDef.Name);
    ...
    for (int j = 0; j < methodCount; j++)
    {
     ...
     DecoderMethodRT.Name = ctx_.generator.GenerateNewNameChinese();
