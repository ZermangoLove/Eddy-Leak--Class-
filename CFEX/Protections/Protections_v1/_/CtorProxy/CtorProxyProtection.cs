...
{
 class CtorProxyProtection : ProtectionPhase
 {
  ...
  public void FirstPhase(Context ctx)
  {
   ...
   txt.proxy.Name = ctx.generator.GenerateNewName();
  ...
  public void SecondFinalPhase(MetadataProcessor.MetadataAccessor accessor, Context ctx)
  {
   ...
   foreach (var i in txt.txts)
   {
    ...
    ctx.logger.Progress(Name + "Writing metadata on : "+i.token);
  ...
  {
   return Context.generator.GenerateNewName();
  }
