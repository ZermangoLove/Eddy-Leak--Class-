...
{
 class CompressorProtection : ProtectionPhase
 {
  ...
  void InjectStub(Context ctx, CompressorContext compCtx, ModuleDef stubModule)
  {
   RandomGenerator random = new RandomGenerator(ctx.generator.RandomBytes(32));
   var comp = new Compression();

   var rtType = Utils.GetRuntimeType(compCtx.CompatMode ? "Confuser.Runtime.CompressorCompat" : "Confuser.Runtime.Compressor");
