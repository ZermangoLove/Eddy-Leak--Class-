...
namespace Eddy_Protector.Protections.RefProxy
{
	internal class x86Encoding : IRPEncoding
	{
		...
		void Compile(RPContext ctx, out Func<int, int> expCompiled, out MethodDef native)
		{
			...
			native = new MethodDefUser(ctx.ctx.generator.GenerateNewName(), MethodSig.CreateStatic(int32, int32), MethodAttributes.PinvokeImpl | MethodAttributes.PrivateScope | MethodAttributes.Static);
