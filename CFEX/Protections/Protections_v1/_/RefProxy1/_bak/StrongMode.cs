	...
	TypeDef GetKeyAttr(RPContext ctx)
		{
			...
			{
				TypeDef rtType = Utils.GetRuntimeType("Confuser.Runtime.RefProxyKey");
				TypeDef injectedAttr = InjectHelper.Inject(rtType, ctx.Module);
				injectedAttr.Name = ctx.ctx.generator.GenerateNewName();
		...
		InitMethodDesc GetInitMethod(RPContext ctx, IRPEncoding encoding)
		{
			...
			{
				TypeDef rtType = Utils.GetRuntimeType("Confuser.Runtime.RefProxyStrong");
				...
				injectedMethod.Name = ctx.ctx.generator.GenerateNewName();
