...
namespace Eddy_Protector.Protections.RefProxy
{
   ...
   //{
   // ctx.runtime_protect.runtime_string.DoEncryption(method, ctx);
   //}
   foreach (MethodDef method in ret.RuntimeMethods)
			{
    //ctx.runtime_protect.runtime_mutation.DoMutation(method, ctx);
    //ctx.runtime_protect.runtime_string.DoEncryption(method,ctx);
    ...
    //ctx.runtime_protect.runtime_bignumber.DoProtect(ctx, method);
   }
