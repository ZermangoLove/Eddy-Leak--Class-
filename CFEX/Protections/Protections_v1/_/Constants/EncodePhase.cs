...
namespace Eddy_Protector.Protections.Constants
{
	class EncodePhase
	{
		...
		public void Execute(CEContext ctx)
		{
			...
			foreach (var entry in ldc)
			{
				...
				{
					EncodeConstant32(moduleCtx, (uint)(int)entry.Key, moduleCtx.Context.CurrentModule.CorLibTypes.Int32, entry.Value);
				}
				else if (entry.Key is long)
				{
					EncodeConstant64(moduleCtx, (uint)((long)entry.Key >> 32), (uint)(long)entry.Key, moduleCtx.Context.CurrentModule.CorLibTypes.Int64, entry.Value);
				}
				else if (entry.Key is float)
				{
					var t = new RTransform();
					t.R4 = (float)entry.Key;
					EncodeConstant32(moduleCtx, t.Lo, moduleCtx.Context.CurrentModule.CorLibTypes.Single, entry.Value);
				}
				else if (entry.Key is double)
				{
					var t = new RTransform();
					t.R8 = (double)entry.Key;
					EncodeConstant64(moduleCtx, t.Hi, t.Lo, moduleCtx.Context.CurrentModule.CorLibTypes.Double, entry.Value);
				}
				else
			...
			encodedBuff = new Compression().Compress(encodedBuff);
			...
			{
				state ^= state >> ctx.Context.AntiTamperEofResult; //12
				state ^= state << 3000; //25
				state ^= state >> 4000; //27
				key[i] = state;
			}
   ...
   MutationHelper.InjectKeys(moduleCtx.InitMethod,
                             ...
                             new[] {
                              ...
                              (int)keySeed,
                              ctx.Context.AntiTamperRegKey //Antitamperkey
                             });
		...
		}//and there i may use encryption for initializer!
		...
		{
			int buffIndex = -1;
			///Tohle to to halvní co se předává jako parametr!
		...
		{
			foreach (var type in context.Context.CurrentModule.GetTypes())
			...
			Dictionary<byte[], List<Tuple<MethodDef, Instruction>>> ldInit)
		{
			...
			var fieldRefs = new HashSet<Instruction>();
				...
				#region ENCODING PART

				//foreach (MethodDef method in moduleCtx.Context.moduleDef.GetTypes().SelectMany(t => t.Methods))
				foreach (MethodDef method in moduleCtx.Context.analyzer.targetCtx.methods_usercode)
				{
					if (!method.HasBody)
						continue;

    moduleCtx.Context.logger.Progress("Getting constants from method: " + method.Name);

					moduleCtx.Elements = 0;
					string elements = "SNPI";
