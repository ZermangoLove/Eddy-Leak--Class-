...
{
	class ConstantsProtection : ProtectionPhase
	{
   ...
   //	if (member is MethodDef)
   //		annotations.Get<ConstantsProtection>(context, member);
   //});

   //ctx.RuntimeCompressor = decomp;

   decomp.Name = context.Context.generator.GenerateNewName();

   ctx.runtime_protect.runtime_controlflow1.DoControlFlow(decomp, ctx);

   ctx.runtime_protect.runtime_methods.Add(decomp); //Add to runtime methods
  ...
  void ProtectRuntimeMethods(Context ctx,CEContext context)
  {
   foreach (var m in context.RuntimeMethods)
   {
    ...
    ctx.runtime_protect.runtime_controlflow2.DoControlFlow(m, ctx);
		...
		{
			IEnumerable<IDnlibDef> members = InjectHelper.Inject(Utils.GetRuntimeType("Confuser.Runtime.Constant"), moduleCtx.Context.CurrentModule.GlobalType, moduleCtx.Context.CurrentModule);
			foreach (IDnlibDef member in members)
			{
				if (member.Name == "Get")
				{
					moduleCtx.Context.CurrentModule.GlobalType.Remove((MethodDef)member);
     member.Name = moduleCtx.Context.generator.GenerateNewName();
   ...
   var dataType = new TypeDefUser("", moduleCtx.Context.generator.GenerateNewName(), moduleCtx.Context.CurrentModule.CorLibTypes.GetTypeRef("System", "ValueType"));
			...
			moduleCtx.DataField = new FieldDefUser(moduleCtx.Context.generator.GenerateNewName(), new FieldSig(dataType.ToTypeSig()))
			{
				IsStatic = true,
			...
			MethodDef decoder = Utils.GetRuntimeType("Confuser.Runtime.Constant").FindMethod("Get");
			decoder.Attributes = MethodAttributes.Static;

   decoder.Name = moduleCtx.Context.generator.GenerateNewName();
    ...
    MutationHelper.InjectKeys(decoderInst,
		...
		{
			decomp.Name = moduleCtx.Context.generator.GenerateNewName();
