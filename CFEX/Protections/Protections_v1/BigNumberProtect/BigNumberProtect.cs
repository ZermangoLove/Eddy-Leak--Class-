...
using Eddy_Protector_Protections.Protections.BigNumberProtection;
using System.Security.Cryptography;

namespace Eddy_Protector_Protections.Protections.BigNumberProtection
{
 public class RC6LiteEncryption
 {
  /// <summary>
  /// Ecrypt for RC6
  /// </summary>
 ...
 {
  public BigNumberProtect bigNumberProtection;
  public Context ctx;
  ...
  {
   var assembly = AssemblyDef.Load("Eddy_Protector_Runtime.dll");
   var type = assembly.ManifestModule.Find("Eddy_Protector_Runtime.Constant_Two", false);
 ...
 public class BigNumberProtect : ProtectionPhase
 {
  ...
  {
   var assembly = AssemblyDef.Load("Eddy_Protector_Runtime.dll");
   var type = assembly.ManifestModule.Find("Eddy_Protector_Runtime.KeysHiderRuntime", false);
  ...
  public void ProtectRuntimeMethodRes(ProtectionContext pCtx)
  {
   ...
   pCtx.ctx.runtime_protect.runtimeControlFlow2.DoControlFlow(DecryptionMethodResources, pCtx.ctx);
  }

 }
}

public class BigNumberRuntimeProtection
{
 ProtectionContext pCtx;
 public BigNumberRuntimeProtection(Context ctx)
 {
  pCtx = new ProtectionContext();
  pCtx.bigNumberProtection = new BigNumberProtect();
  pCtx.ctx = ctx;
  pCtx.GetDecryptionMethod();
  pCtx.bigNumberProtection.GetDecryptionMethodRes(ctx);
  pCtx.Encryptor = new RC6LiteEncryption();
  pCtx.Encryption = new Encryption();
  pCtx.Encryption.ctx = ctx;
  pCtx.Encryption.pCtx = pCtx;
 ...
 public void DoProtect(Context ctx, MethodDef method)
 {
  ...
  pCtx.bigNumberProtection.DoEncrypt(pCtx, ctx, method);
  pCtx.bigNumberProtection.DoEncryptSpecified(method, ctx, pCtx.UsedKeys);
