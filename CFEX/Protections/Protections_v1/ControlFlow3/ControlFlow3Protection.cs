...
namespace Eddy_Protector_Protections.Protections.ControlFlow3
{
 public class ControlFlow3Protection : ProtectionPhase
 {
  public override string Author => Engine.Author;
  public override string Description => "Control flow 3";
  public override string Id => "ControlFlow3";
  public override string Name => "ControlFlow3";

  public override void Execute(Context ctx)
  {

   var p = new RuntimeControlFlow3Protection();
   foreach(var m in ctx.analyzer.targetCtx.methods_usercode)
   {
    p.DoControlFlow(m, ctx);
   }
 ...
 public class RuntimeControlFlow3Protection
 {
  ...
  {
   obfMethods = CreateMethods(ctx, ctx.CurrentModule);
   ...
   //{
   // var mm = m.Key;
   // ctx.runtime_protect.runtime_intmath.DoMutation(mm, ctx);
   //}
  }
  ...
  public static Dictionary<MethodDef, Tuple<int[], int[]>> CreateMethods(Context ctx, ModuleDef loadedMod)
  {
   ...
   for (int i = 0; i < 4; i++)
   {
    ...
    methodDefs1.Name = ctx.generator.GenerateNewNameChinese();
   ...
   type1.Name = ctx.generator.GenerateNewNameChinese();
