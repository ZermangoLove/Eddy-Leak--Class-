...
//
using Eddy_Protector.Virtualization.AST;
using Eddy_Protector.Virtualization.AST.IR;
using Eddy_Protector.Virtualization.VM;


namespace Eddy_Protector.Virtualization.VMIR.Transforms
{
	public class InvokeTransform : ITransform
	{
		...
		private void ProcessECall(IRInstrList instrs, IRInstruction instr, int index, IRTransformer tr)
		{
			...
			uint opCode = 0;
			var constrainType = ((InstrCallInfo)instr.Annotation).ConstrainType;
			if (instr.OpCode == IROpCode.__CALL) opCode = tr.VM.Runtime.VCallOps.ECALL_CALL;
			...
			else if (instr.OpCode == IROpCode.__NEWOBJ) opCode = tr.VM.Runtime.VCallOps.ECALL_NEWOBJ;
