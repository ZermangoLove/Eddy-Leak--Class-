...
namespace Protector.Protections.ControlFlow
{
 class RTControlFlow
 {
  ...
  ControlFlow control;
  public RTControlFlow(ModuleDef mod)
  {
   control = new ControlFlow(mod);
  }

  public MethodDef AddControlFlow(MethodDef method, ProtectorContext context)
  {
 ...
 class ControlFlow
 {
  ...
  ModuleDef module;
  public ControlFlow(ModuleDef mod)
  {
   module = mod;
  }
  ...
  {
   obfMethods = CreateMethods(ctx, module);
  ...
  public static Dictionary<MethodDef, Tuple<int[], int[]>> CreateMethods(ProtectorContext ctx, ModuleDef loadedMod)
  {
   ...
   for (int i = 0; i < 4; i++)
   {
    ...
    methodDefs1.Name = ctx.random_generator.GenerateString();
   ...
   type1.Name = ctx.random_generator.GenerateString();
