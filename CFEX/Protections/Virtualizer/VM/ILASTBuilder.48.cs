	...
	public class ILASTBuilder
	{
		...
		private void BuildPhiNodes()
		{
			foreach (var statePair in blockStates)
			{
				...
				{
					for (var varIndex = 0; varIndex < state.BeginStack.Length; varIndex++)
					{
						var phi = new ILASTPhi { Variable = state.BeginStack[varIndex] };
						phi.SourceVariables = new ILASTVariable[block.Sources.Count];
						for (var i = 0; i < phi.SourceVariables.Length; i++) phi.SourceVariables[i] = blockStates[block.Sources[i]].ASTTree.StackRemains[varIndex];
