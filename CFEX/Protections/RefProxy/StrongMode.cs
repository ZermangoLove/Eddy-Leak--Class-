...
namespace Protector.Protections.RefProxy
{
   ...
   int pop;
		...
		{
			var method = new MethodDefUser(ctx.ctx.random_generator.GenerateString(), sig);
	...
	TypeDef GetKeyAttr(RPContext ctx)
		{
			...
			{
				TypeDef rtType = DnLibHelper.GetRuntimeType("Protector.Runtime.RefProxyKey");
				TypeDef injectedAttr = InjectHelper.Inject(rtType, ctx.Module);
				injectedAttr.Name = ctx.ctx.random_generator.GenerateString();
		...
		InitMethodDesc GetInitMethod(RPContext ctx, IRPEncoding encoding)
		{
			...
			{
				TypeDef rtType = DnLibHelper.GetRuntimeType("Protector.Runtime.RefProxyStrong");
				...
				injectedMethod.Name = ctx.ctx.random_generator.GenerateString();
