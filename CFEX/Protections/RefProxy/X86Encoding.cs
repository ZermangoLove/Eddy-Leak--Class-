...
namespace Protector.Protections.RefProxy
{
	internal class x86Encoding : IRPEncoding
	{
		...
		void Compile(RPContext ctx, out Func<int, int> expCompiled, out MethodDef native)
		{
			...
			native = new MethodDefUser(ctx.ctx.random_generator.GenerateString(), MethodSig.CreateStatic(int32, int32), MethodAttributes.PinvokeImpl | MethodAttributes.PrivateScope | MethodAttributes.Static);
			...
			if (!addedHandler)
			{
				ctx.ctx.CurrentModuleWriterListener.OnWriterEvent += InjectNativeCode;
				addedHandler = true;
			}
		}
